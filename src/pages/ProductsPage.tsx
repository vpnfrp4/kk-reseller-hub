...
export default function ProductsPage() {
  ...
  const handleBuy = (product: typeof mockProducts[0]) => {
    if ((user?.balance || 0) < product.wholesalePrice) {
      toast.error("Insufficient balance. Please top up your wallet.");
      return;
    }
    // Mock instant delivery
    toast.success(
      <div>
        <p className="font-semibold">Purchase Successful!</p>
        <p className="text-xs mt-1 font-mono">Credentials: demo@vpn.com / Xk9mP2q</p>
      </div>,
      { duration: 8000 }
    );
  };
          ...
          >
            ...
            <div className="mt-auto space-y-3">
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-xs text-muted-foreground line-through">{product.retailPrice.toLocaleString()} MMK</p>
                  <p className="text-xl font-bold font-mono text-primary">{product.wholesalePrice.toLocaleString()} <span className="text-xs text-muted-foreground">MMK</span></p>
                </div>
