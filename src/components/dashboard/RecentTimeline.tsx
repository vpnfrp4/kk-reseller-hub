...
const itemVariants = {
  ...
  visible: (i: number) => ({
    opacity: 1, x: 0,
    transition: { type: "spring", stiffness: 350, damping: 26, delay: i * 0.07 },
  }),
};
    ...
    <div className="cd-card cd-reveal">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          <div className="w-1.5 h-6 rounded-full bg-gradient-to-b from-success to-success/40" />
          <h2 className="text-sm lg:text-base font-extrabold text-foreground tracking-tight">Recent Timeline</h2>
        </div>
        <button
          ...
          {orders.slice(0, 5).map((order: any, i: number) => {
            ...
            return (
              <motion.button
                ...
                className={cn(
                  "w-full flex items-start gap-3 p-3 rounded-xl text-left group",
                  "hover:bg-secondary/40 transition-colors duration-200"
                )}
              >
                {/* Status dot with line connector */}
                ...
                <ArrowRight className="w-3.5 h-3.5 text-muted-foreground/30 group-hover:text-primary/60 mt-1 shrink-0 transition-colors" />
              </motion.button>
            );
          })}
        </div>
