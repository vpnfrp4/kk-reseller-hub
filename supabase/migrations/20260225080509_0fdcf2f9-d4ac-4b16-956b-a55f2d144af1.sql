...
BEGIN
  ...
  v_needs_admin := (p_fulfillment_mode IN ('manual', 'custom_username', 'imei'));
  ...
  INSERT INTO public.orders (user_id, product_name, credential_id, credentials, price, status, fulfillment_mode)
    VALUES (
      ...
      v_total_price,
      CASE
        WHEN v_is_manual THEN 'pending_review'
  ...
  IF v_needs_admin THEN
    ...
    v_notif_title := CASE p_fulfillment_mode
      WHEN 'manual' THEN '🔔 Manual Order Pending'
      WHEN 'custom_username' THEN '👤 Username Order Pending'
      WHEN 'imei' THEN '📱 IMEI Order Pending'
      ELSE '🔔 Order Pending'
    END;
