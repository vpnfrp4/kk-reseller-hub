...
BEGIN
  ...
  -- Skip stock check for manual products
  ...
  -- For manual products, skip credential assignment
  IF v_is_manual THEN
    v_credentials_list := ARRAY['Pending manual fulfillment'];
  ELSE
    -- Grab credentials for non-manual products
  ...
  -- Decrement stock only for non-manual products
