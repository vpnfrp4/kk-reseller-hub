...
DECLARE
  ...
  v_cost_per_1000 BIGINT;
  v_sell_per_1000 BIGINT;
BEGIN
  ...
  IF v_ptype = 'api' AND v_product.api_rate IS NOT NULL THEN
    ...
    -- Margin hierarchy: service > category > global
    ...
    -- Formula:
    -- 1. cost_per_1000 = ceil(api_rate * exchange_rate)
    -- 2. sell_per_1000 = ceil(cost_per_1000 * (1 + margin/100))
    -- 3. unit_price = sell_per_1000 / 1000
    -- 4. total_price = ceil(unit_price * quantity)
    v_cost_per_1000 := ceil(v_product.api_rate * v_usd_rate);
    v_sell_per_1000 := ceil(v_cost_per_1000 * (1 + v_margin / 100.0));
    v_total_price := ceil((v_sell_per_1000 / 1000.0) * p_quantity);
    v_unit_price := v_sell_per_1000;  -- store sell_per_1000 as reference

    -- Provider cost (no margin)
    v_provider_cost := ceil((v_cost_per_1000 / 1000.0) * p_quantity);
  ...
  CASE v_ptype
    WHEN 'digital' THEN v_order_status := 'delivered'; v_order_mode := 'instant';
    WHEN 'imei' THEN v_order_status := 'pending'; v_order_mode := COALESCE(p_fulfillment_mode, 'manual');
    WHEN 'manual' THEN v_order_status := 'pending_review'; v_order_mode := 'manual';
    WHEN 'api' THEN v_order_status := 'api_pending'; v_order_mode := 'api';
    ELSE v_order_status := 'pending'; v_order_mode := 'manual';
  END CASE;
  ...
  VALUES (
    ...
    p_product_id, v_ptype,
    CASE WHEN v_ptype = 'digital' THEN v_credential_ids[1] ELSE NULL END,
    array_to_string(v_credentials_list, E'\n'),
    v_total_price, v_order_status, v_order_mode,
    p_imei_number, p_custom_fields,
    v_provider_cost, v_profit
  )
